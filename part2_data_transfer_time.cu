...
// GPU Matrix Multiplication Kernel (Single Thread, Single Block)
__global__ void MatrixMulKernel(float* M, float* N, float* P, int Width) {
    // Since there's only one thread, we iterate over the entire matrix manually
...
int main() {
    ...
    for (int i = 0; i < numOfSizes; i++) {
        ...
        // Configure GPU execution to use **only one block and one thread**
